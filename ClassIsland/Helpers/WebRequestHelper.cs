...
public class WebRequestHelper
{
    ...
    public static async Task<T> GetJson<T>(Uri uri, int retries = 3, CancellationToken? cancellationToken = null,
        bool verifySign = false, string? publicKey = null)
    {
        var logger = IAppHost.TryGetService<ILogger<WebRequestHelper>>();
        cancellationToken = cancellationToken ?? CancellationToken.None;
        if (retries > MaxRetries) throw new ArgumentException("重试次数超过规定最大重试次数。", nameof(retries));
        ...
        for (var i = 0; i <= retries; i++)
            try
            {
                ...
                if (verifySign && publicKey != null)
                {
                    ...
                    if (!valid) throw new GeneralSecurityException("数据签名校验失败。");
                }
            ...
            catch (Exception ex)
            {
                ...
                retryTime *= 2;
                if (i < retries)
                    await Task.Run(() => cancellationToken.Value.WaitHandle.WaitOne(retryTime),
    ...
    public static async Task<T> SaveJson<T>(Uri uri, string path, int retries = 3,
        CancellationToken? cancellationToken = null, bool verifySign = false, string? publicKey = null)
    {
