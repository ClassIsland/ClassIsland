...
/// </summary>
///
[SettingsPageInfo("classisland.plugins", "插件", PackIconKind.ToyBrickOutline, PackIconKind.ToyBrick,
...
public partial class PluginsSettingsPage : SettingsPageBase
{
    ...
    public PluginsSettingsPage(IPluginService pluginService, IPluginMarketService pluginMarketService,
        SettingsService settingsService, ILogger<PluginsSettingsPage> logger)
    {
        InitializeComponent();
        DataContext = this;
    ...
    private async Task UpdateReadmeDocument()
    {
        ...
        try
        {
            ...
            DocumentLoadingCancellationTokenSource = new CancellationTokenSource();
    ...
    private async void MenuItemPackPlugin_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        var dialog = new SaveFileDialog
        {
            ...
            Filter =
                $"ClassIsland 插件包(*{IPluginService.PluginPackageExtension})|*{IPluginService.PluginPackageExtension}"
        };
    ...
    private void ButtonInstallFromLocal_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            File.Copy(dialog.FileName,
                Path.Combine(Services.PluginService.PluginsPkgRootPath, Path.GetFileName(dialog.FileName)), true);
            RequestRestart();
        }
    ...
    private void MenuItemOpenPluginConfigFolder_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        Process.Start(new ProcessStartInfo
        {
            FileName = Path.Combine(Services.PluginService.PluginConfigsFolderPath,
    ...
    private async void ButtonBaseRefreshPlugins_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        if (FindResource("PluginSource") is CollectionViewSource source) source.View.Refresh();
    }
    ...
    private void InstallPlugin(string id)
    {
        ...
        foreach (var i in resolvedPlugins) PluginMarketService.RequestDownloadPlugin(i.Manifest.Id);
    }

    private void ResolveDependencies(PluginInfo plugin, List<PluginInfo> resolvedPlugins, List<string> missingPlugins)
    {
        if (IPluginService.LoadedPluginsIds.Contains(plugin.Manifest.Id) || resolvedPlugins.Contains(plugin)) return;
        resolvedPlugins.Add(plugin);
        foreach (var i in plugin.Manifest.Dependencies)
        {
            ...
            if (dep == null)
            {
                if (i.IsRequired && !IPluginService.LoadedPluginsIds.Contains(i.Id)) missingPlugins.Add(i.Id);
                continue;
            }
    ...
    private void Grid_Drop(object sender, DragEventArgs e)
    {
        ...
        if (e.Data.GetDataPresent(DataFormats.FileDrop))
        {
            ...
            try
            {
                File.Copy(fileName, Path.Combine(Services.PluginService.PluginsPkgRootPath, Path.GetFileName(fileName)),
                    true);
    ...
    {
        if (PluginMarketService.MergedPlugins.Any(x => x.Value.DownloadProgress?.IsDownloading == true)) return;
        RequestRestart();
    }
