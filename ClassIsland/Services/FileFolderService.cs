...
public class FileFolderService(SettingsService settingsService, ILogger<FileFolderService> logger) : IHostedService
{
    ...
    {
        foreach (var i in Folders.Where(i => !Directory.Exists(i))) Directory.CreateDirectory(i);
    }
    ...
    {
        if (!SettingsService.Settings.IsAutoBackupEnabled) return;
        ...
        if (!Directory.Exists(Path.Combine(App.AppRootFolderPath, "Backups"))) return;

        if (SettingsService.Settings.AutoBackupLimit <= 0) return;
        var outdatedBackups = Directory.EnumerateDirectories(Path.Combine(App.AppRootFolderPath, "Backups"), "Auto_*")
            .OrderByDescending(Directory.GetLastWriteTime).Skip(SettingsService.Settings.AutoBackupLimit).ToList();
        foreach (var i in outdatedBackups) Directory.Delete(i, true);
    }
    ...
    public static async Task CreateBackupAsync(bool isAuto = false, string? filename = null, string? rootPath = null)
    {
        ...
        var backupFilename = string.IsNullOrWhiteSpace(filename)
            ? $"Backup_{DateTime.Now:yy-MMM-dd_HH-mm-ss}.zip"
            : filename + ".zip";
        if (isAuto) backupFilename = "Auto_" + backupFilename;
        ...
        await Task.Run(() =>
        {
            ...
            foreach (var file in backupFiles)
            {
                ...
                if (File.Exists(filePath)) archive.CreateEntryFromFile(filePath, file);
            }
