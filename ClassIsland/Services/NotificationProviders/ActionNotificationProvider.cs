...
public class ActionNotificationProvider : NotificationProviderBase
{
    ...
    public ActionNotificationProvider(INotificationHostService notificationHostService, IActionService actionService)
    {
        ...
        ActionService.RegisterActionHandler("classisland.showNotification",
            (o, s) => AppBase.Current.Dispatcher.Invoke(() => ActionHandler(o, s)));
    }

    private void ActionHandler(object? o, string guid)
    {
        if (o is not NotificationActionSettings settings) return;
        ShowNotification(new NotificationRequest
        {
            ...
            OverlayContent = string.IsNullOrWhiteSpace(settings.Content) || settings.ContentDurationSeconds <= 0
                ? null
                : NotificationContent.CreateSimpleTextContent(settings.Content, x =>
                {
