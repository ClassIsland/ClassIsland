...
public static class SpeechProviderRegistryExtensions
{
    ...
    public static IServiceCollection AddSpeechProvider<TSpeechProvider>(this IServiceCollection services)
    ...
    /// <param name="services"><see cref="IServiceCollection"/> 服务集合</param>
    /// <returns>原来的 <see cref="IServiceCollection"/> 对象</returns>
    public static IServiceCollection AddSpeechProvider<TSpeechProvider, TSpeechProviderSettingsControl>(
    ...
    {
        if (provider.GetCustomAttributes(false).FirstOrDefault(x => x is SpeechProviderInfo) is not SpeechProviderInfo
            info) throw new ArgumentException($"无法注册语音提供方，因为这个语音提供方 {provider.FullName} 没有注册信息。");
        ...
        if (settings != null) info.SettingsControlType = settings;
