...
/// 
/// </summary>
public class TouchFixedContextDragBehavior : StyledElementBehavior<Control>
{
    ...
    {
        AssociatedObject?.ShowToast("(debug) Released!");
        _triggerEvent = null;
        _lock = false;
    }
    ...
    private void AssociatedObject_PointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        if (properties.IsLeftButtonPressed)
        {
            ...
            {
                _isTouchDragging = e.Pointer.Type == PointerType.Touch;
                AssociatedObject?.ShowToast("(debug) Begin Drag");
                if (_isTouchDragging && control.FindAncestorOfType<TouchDragThumb>() == null)
                {
                    return;
                }
    ...
    private async void AssociatedObject_PointerMoved(object? sender, PointerEventArgs e)
    {
            ...
            _triggerEvent is not null)
        {
            ...
            if (Math.Abs(diff.X) > horizontalDragThreshold || Math.Abs(diff.Y) > verticalDragThreshold)
            {
                ...
                AssociatedObject?.ShowToast("(debug) DoDragDrop");
                await DoDragDrop(e, context);

                AssociatedObject?.ShowToast("(debug) AfterDragDrop");
