...
var installation = Directory.GetDirectories(root)
    ...
    .ThenBy(x =>
    {
        ...
        if (split.Length <= 1) return new Version();
...
if (installation == null || !File.Exists(Path.Combine(installation, "ClassIsland.exe")))
{
    PInvoke.MessageBox(HWND.Null,
        "找不到有效的 ClassIsland 版本，可能是安装已损坏。请在 https://classisland.tech/download 重新下载并安装 ClassIsland。",
...
foreach (var i in args) startInfo.ArgumentList.Add(i);
