    ...
    GeoCoordinateWatcherBase, ILocationEvents, IDisposable
{
    ...
    private void CreateHandler(object state)
    {
        lock (InternalSyncObject)
        {
            ...
            if (null != m_location)
            {
                ...
                var accuracy = desiredAccuracy == GeoPositionAccuracy.Default
    ...
    public override GeoPosition<GeoCoordinate> Position => IsStarted && Status == GeoPositionStatus.Ready
        ? m_position
    ...
    private void GetLocationHelper()
    {
        lock (InternalSyncObject)
        {
            if (m_location != null && m_curStatus == GeoPositionStatus.Ready)
            {
                ...
                if (m_location.GetReport(ref reportType, out report) == 0) HandleLocationChangedEvent(report);
            }
        }
    }
    ...
    private void HandleLocationChangedEvent(ILocationReport locationReport)
    {
        ...
        if (m_latLongStatus != ReportStatus.NotSupported)
        {
            ...
            if (latLongReport != null)
            {
                    ...
                    latLongReport.GetLongitude(out longitude) == 0)
                {
                    ...
                    coordinate = new GeoCoordinate(latitude, longitude, altitude, errorRadius, altitudeError, speed,
                        course);
                }
            }
        }
        ...
        if (!coordinate.IsUnknown || !address.IsUnknown)
        {
            ...
            //
            if (!coordinate.IsUnknown && !address.IsUnknown) coordinate.m_address = address;
    ...
    private bool EnterProcessing()
    {
        ...
        if (Interlocked.Increment(ref m_positionEventsProcessingCount) == int.MaxValue - 1)
        {
