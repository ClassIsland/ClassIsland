...
internal sealed class GeoLocationProviderInternal : GeoLocationProviderBase, ILocationEvents, IDisposable
{
    ...
    private void CreateHandler(object state)
    {
        lock (InternalSyncObject)
        {
            ...
            if (null != m_location)
            {
                ...
                //
                var desiredAccuracy = (GeoLocationAccuracy)state;
                var accuracy = desiredAccuracy == GeoLocationAccuracy.Low
                ...
                //
                if (m_location.RegisterForReport(this, ref reportKey, 0) == 0) m_latLongRegistered = true;
            ...
            var eventDone = m_eventCreateDone;
            if (eventDone != null) eventDone.Set();
        }
    }
    ...
    public override GeoLocationStatus Status =>
        m_curStatus == GeoLocationStatus.Ready && m_curLocation == GeoLocation.Unknown
            ? GeoLocationStatus.NoData
            : m_curStatus;
    ...
    public override GeoLocation Location
    {
        get
        {
            ...
            //
            if (m_started &&
                !(GeoLocationStatus.NoData == m_curStatus || GeoLocationStatus.NoPermissions == m_curStatus))
    ...
    private void GetInitialLocationData(object state)
    {
        lock (InternalSyncObject)
        {
            ...
            if (m_location != null)
            {
                ...
                {
                    var reportType = m_latLongStatus != ReportStatus.NotSupported
                        ? LocationReportKey.LatLongReport
                        : LocationReportKey.CivicAddressReport;
    ...
    private void GetLocationHelper()
    {
        for (var i = 0; i < 5; i++)
        {
            if (m_location == null || m_curStatus != GeoLocationStatus.Ready ||
                m_curLocation != GeoLocation.Unknown) break;
    ...
    void ILocationEvents.OnPositionStatusChanged(ref Guid reportType, ReportStatus newStatus)
    {
            ...
            reportType.Equals(LocationReportKey.CivicAddressReport))
        {
            ...
            else if (reportType.Equals(LocationReportKey.CivicAddressReport)) m_civicAddrStatus = newStatus;
    ...
    private void HandleLocationChangedEvent(ILocationReport locationReport)
    {
        ...
        if (m_latLongStatus != ReportStatus.NotSupported)
        {
            ...
            if (latLongReport != null)
            {
                    ...
                    latLongReport.GetLongitude(out longitude) == 0)
                {
                    ...
                    coordinate = new GeoCoordinate(latitude, longitude, errorRadius, altitude, altitudeError, 0.0, 0.0);
                }
            }
        }
        ...
        if (countryRegion != string.Empty)
        {
            ...
            if (address1 != string.Empty || address2 != string.Empty || city != string.Empty ||
                ...
                address = new CivicAddress(address1, address2, string.Empty, city, countryRegion, string.Empty,
        ...
        {
            var heading = GetDoubleProperty(locationReport, LocationPropertyKey.Heading);
            ...
            if (m_started)
            {
                ...
                if (GeoLocationStatus.Ready == Status)
                {
                    ...
                    //
                    if (Status != prevStatus) OnStatusChanged(new GeoLocationStatusChangedEventArgs(m_curStatus));
    ...
    private void HandleLocationStatusChangedEvent(ReportStatus newStatus)
    {
        ...
        if (m_civicAddrRegistered && m_latLongStatus != ReportStatus.NotSupported)
            if (m_civicAddrRegistered)
            {
                ...
                if (m_location.UnregisterForReport(ref reportType) == 0) m_civicAddrRegistered = false;
            }
        ...
        lock (InternalSyncObject)
        {
            if (m_location != null)
            {
                ...
                if (m_latLongRegistered)
                {
                    ...
                    if (m_location.UnregisterForReport(ref reportType) == 0) m_latLongRegistered = false;
                }
