...
{
    private static DateTime TimeSpanToDateTime(TimeSpan timeSpan)
    {
    ...
    public static Profile ToClassIslandObject(this CsesSharp.Models.Profile profile, Profile? mergeProfile = null)
    {
        ...
        foreach (var i in result.Subjects) subjectsCache[i.Value.Name] = i.Key;
        foreach (var i in profile.Subjects)
        {
            ...
            result.Subjects[id].Initial = string.IsNullOrWhiteSpace(i.SimplifiedName)
                ? result.Subjects[id].Initial
                : i.SimplifiedName;
            result.Subjects[id].TeacherName =
                string.IsNullOrWhiteSpace(i.Teacher) ? result.Subjects[id].TeacherName : i.Teacher;
        }
        ...
        foreach (var i in profile.Schedules)
        {
            var timeLayoutKey = result.TimeLayouts.FirstOrDefault(x =>
            {
                ...
                if (validTimeLayout.Count != i.Classes.Count) return false;
            ...
            for (var j = 0; j < i.Classes.Count; j++)
            {
                ...
                if (timeLayoutKey != null) continue;
    ...
    public static CsesSharp.Models.Profile ToCsesObject(this Profile profile,
        CsesSharp.Models.Profile? mergeProfile = null)
    {
        ...
        foreach (var i in profile.ClassPlans.Where(x => x.Value is
                 {
                     TimeLayout: not null, IsEnabled: true, TimeRule.WeekCountDivTotal: <= 2,
                     TimeRule.WeekCountDiv: <= 2
                 }).ToList())
        {
