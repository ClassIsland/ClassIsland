...
$iosDisplayVersionText = Read-RepositoryFile "tools/ci/ios-display-version.sh"
$iosDisplayVersionTestsText = Read-RepositoryFile "tools/ci/test-ios-display-version.sh"
...
Assert-True ($releaseWorkflowText.Contains("source ./tools/ci/ios-display-version.sh")) "The iOS metadata job must use the shared display-version resolver."
Assert-True ($releaseWorkflowText.Contains("git describe --tags --abbrev=0 --match '[0-9]*.[0-9]*.[0-9]*' HEAD")) "PR and branch iOS builds must follow the nearest Android numeric release tag."
Assert-True ($releaseWorkflowText.Contains('version_source="$DISPATCH_RELEASE_TAG"')) "Release dispatch iOS versions must follow the Android release tag."
Assert-True ($releaseWorkflowText.Contains('app_version="$(resolve_ios_display_version "$version_source")"')) "All iOS version sources must be normalized to Apple's three-component display version."
Assert-True (-not $releaseWorkflowText.Contains('app_version="0.0.${GITHUB_RUN_NUMBER}"')) "iOS display versions must not depend on the GitHub run number."
Assert-True ($releaseWorkflowText.Contains('checkout_ref="$(git rev-parse HEAD)"')) "The iOS worker must receive the immutable commit resolved by the metadata job."
...
Assert-True ($workerWorkflowText.Contains("bash ./tools/ci/test-ios-display-version.sh")) "The iOS worker must run display-version validation tests."
Assert-True ($workerWorkflowText.Contains('artifact_name="ClassIsland-iOS-${APPLICATION_DISPLAY_VERSION}-build.${APPLICATION_BUILD_NUMBER}-ios-arm64-unsigned-attempt.${GITHUB_RUN_ATTEMPT}"')) "Non-release iOS artifacts must include display version, build number, and rerun attempt."
...
Assert-True ($workerWorkflowText.Contains('name: ${{ steps.artifact_metadata.outputs.artifact_name }}')) "The iOS worker must upload the resolved artifact name."
Assert-True ($workerWorkflowText.Contains("overwrite: true")) "The iOS worker must replace a fixed release artifact safely when a workflow run is retried."
Assert-True (-not $workerWorkflowText.Contains("GITHUB_STEP_SUMMARY") -and -not $workerWorkflowText.Contains("Write build summary")) "The iOS worker must not write a redundant IPA job summary."
...
Assert-True ($ipaVerificationText.Contains("<ipa-path> <application-id> <runtime-identifier> <display-version> <build-number>")) "IPA verification must document its five required arguments."
Assert-True ($ipaVerificationText.Contains("app_display_version") -and $ipaVerificationText.Contains("extension_display_version") -and $ipaVerificationText.Contains("CFBundleShortVersionString")) "IPA verification must validate the app and extension display versions."
...
Assert-True ($iosDisplayVersionText.Contains("resolve_ios_display_version") -and $iosDisplayVersionText.Contains("assert_ios_bundle_display_version")) "iOS display-version validation must expose testable resolver and assertion functions."
Assert-True ($iosDisplayVersionTestsText.Contains("resolve_ios_display_version 2.1.0.1") -and $iosDisplayVersionTestsText.Contains("resolve_ios_display_version ios-v2.1.0.1")) "iOS display-version tests must cover Android and iOS tag formats."
Assert-True ($iosDisplayVersionTestsText.Contains('assert_ios_bundle_display_version app 2.1.1 2.1.0')) "iOS display-version tests must reject an app mismatch."
...
Assert-True ($nukeBuildText.Contains('using Nuke.Common.Tooling;')) "NUKE iOS publish must import the ToolOptions extension methods."
