...
Assert-True ($releaseWorkflowText.Contains("resolve_release_version")) "The unified Build workflow must normalize tag versions before an iOS build."
Assert-True ($releaseWorkflowText.Contains('resolve_release_version "$DISPATCH_PRIMARY_VERSION"')) "Release dispatch versions must be normalized before an iOS build."
...
Assert-True ($releaseWorkflowText.Contains("DISPATCH_PRIMARY_VERSION: `${{ inputs.primary_version }}")) "A release dispatch must use the selected primary version for the iOS display version."
Assert-True ($releaseWorkflowText.Contains("PULL_REQUEST_HEAD_SHA: `${{ github.event.pull_request.head.sha }}")) "Pull-request iOS builds must use the pull-request head commit."
...
Assert-True ($releaseWorkflowText.Contains("DISPATCH_PRIMARY_VERSION: `${{ inputs.primary_version }}")) "The release iOS build must use the selected primary version."
