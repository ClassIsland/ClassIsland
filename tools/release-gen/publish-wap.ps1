...
if ($isReleaseSigning) {
    ...
    $manifestRaw.Replace('Publisher="CN=ClassIsland_TestSigning"', $env:isTestMode ? $ciTestPublisher : $releasePublisher) | Set-Content ./ClassIsland.Packaging/Package.appxmanifest
}
...
# build
if ($isReleaseSigning) {
    msbuild .\ClassIsland.Packaging\ClassIsland.Packaging.wapproj /p:Configuration=Release_MSIX /p:UapAppxPackageBuildMode=SideloadOnly /p:AppxBundle=Never /p:GenerateAppxPackageOnBuild=true /p:AppxPackageDir="Packages" /p:RuntimeIdentifiers="win-x64" /p:Platform="x64" /p:PlatformTarget="x64" /p:PublishBuilding=true
} else {
...
Copy-Item ClassIsland.Packaging\Packages\*\*.appx .\out -Force
